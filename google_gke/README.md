...
| <a name="input_node_pools_enable_gcfs"></a> [node\_pools\_enable\_gcfs](#input\_node\_pools\_enable\_gcfs) | Map containing node pools guest accelerator. Each node pool's name is the key. See locals.tf for defaults. | `map(bool)` | <pre>{<br>  "tf-default-node-pool": false<br>}</pre> | no |
